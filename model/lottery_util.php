...
class LotteryUtil {
    ...
    public static function calMaxMiss($file,$calfunc,$settings = array()){
        $min_count = isset($settings['min_count']) ? intval($settings['min_count']) : 0;
        $show_items = isset($settings['show_items']) ? $settings['show_items'] : false;
        $handle = fopen($file, 'r');
        if(!$handle){
        ...
        while(true){
            ...
            list($item_date,$item_code,$value) = call_user_func_array($calfunc,array($line));
            if(!isset($last)){
                $last = $value;
            }
            if($last === $value){
